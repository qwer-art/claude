...
**重要**：所有文件下载到**当前工作目录**下，使用环境变量 `{{当前工作目录}}` 作为根路径。

```bash
mkdir -p {{当前工作目录}}/raw_data/{项目名称}/code
mkdir -p {{当前工作目录}}/raw_data/{项目名称}/paper
```

所有内容（代码+论文）统一放在 `raw_data/{项目名称}/` 下：
- 代码：直接 clone 到 `raw_data/{项目名称}/code/`
- 论文：下载到 `raw_data/{项目名称}/paper/`

**目录结构说明**：
```
raw_data/{项目名称}/     ← 使用 git 管理（.git 在此层级）
├── code/               ← 不用 git 管理（在 .gitignore 中排除）
└── paper/              ← 不用 git 管理（在 .gitignore 中排除）
```

- `raw_data/{项目名称}/` 目录本身使用 git 管理，用于记录项目元信息
- `code/` 和 `paper/` 子目录不需要 git 跟踪，应在 `.gitignore` 中排除
...
**GitHub 仓库克隆：**
```bash
git clone https://github.com/username/repo.git {{当前工作目录}}/raw_code/{项目名称}/code/
```
- 支持完整的 GitHub URL
...
```bash
# 对于模型仓库
git clone https://huggingface.co/username/model-name {{当前工作目录}}/raw_code/{项目名称}/code/

# 或使用 huggingface-cli (如果安装)
huggingface-cli download username/model-name --local-dir {{当前工作目录}}/raw_code/{项目名称}/code/
```
...
**GitLab 仓库克隆：**
```bash
git clone https://gitlab.com/username/repo.git {{当前工作目录}}/raw_code/{项目名称}/code/
```
- 完整的 GitLab URL 支持
- 支持私有仓库（需要认证）
...
💡 下一步建议：
- 使用 /paper-reader 分析论文：/paper-reader {{当前工作目录}}/raw_code/{项目名称}/{论文文件}
- 使用 /probe_code 分析代码结构：/probe_code {{当前工作目录}}/raw_code/{项目名称}
- 使用 /refine 深入分析细节
```
